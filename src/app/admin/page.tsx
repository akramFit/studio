...
import { Users, Inbox, GalleryHorizontal } from "lucide-react";
...
const AdminDashboard = () => {
  const { user } = useAuth();
  const [stats, setStats] = useState({ pendingOrders: 0, activeClients: 0, galleryItems: 0 });
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const [ordersSnapshot, clientsSnapshot, gallerySnapshot] = await Promise.all([
          getDocs(ordersQuery),
    ...
    <div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{loading ? "..." : stats.galleryItems}</div>
            <p className="text-xs text-muted-foreground">Showcased in your gallery</p>
          </CardContent>
        </Card>
      </div>
